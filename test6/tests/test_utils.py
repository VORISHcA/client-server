...
class TestSocket:
    CONFIGS = utils.load_configs(True)
...
class Tests(unittest.TestCase):
    ...
    def test_send_message(self):
        ...
        utils.send_message(test_socket, self.test_message_send, self.CONFIGS)
        self.assertEqual(test_socket.encoded_message, test_socket.received_message)
        with self.assertRaises(Exception):
            utils.send_message(test_socket, test_socket, self.CONFIGS)

    def test_get_message(self):
        test_sock_ok = TestSocket(self.test_success_receive)
        test_sock_err = TestSocket(self.test_error_receive)
        self.assertEqual(utils.get_message(test_sock_ok, self.CONFIGS), self.test_success_receive)
        self.assertEqual(utils.get_message(test_sock_err, self.CONFIGS), self.test_error_receive)
